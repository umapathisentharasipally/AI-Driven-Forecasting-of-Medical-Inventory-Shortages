...
def build_inventory_filter(filters: dict) -> dict:
    ...
    if filters.get("search"):
        safe_search = re.escape(str(filters["search"]).strip())
        query["$or"] = [
            {"item_id": {"$regex": safe_search, "$options": "i"}},
            {"item_name": {"$regex": safe_search, "$options": "i"}},
            {"facility_id": {"$regex": safe_search, "$options": "i"}},
            {"category": {"$regex": safe_search, "$options": "i"}},
            {"unit_of_measure": {"$regex": safe_search, "$options": "i"}},
        ]
