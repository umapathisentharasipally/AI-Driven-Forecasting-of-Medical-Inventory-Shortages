import pandas as pd
from src.utils.constants import TARGET_COLUMN, DATE_COLUMN
...
def validate_inventory_schema(df: pd.DataFrame, require_target: bool = True) -> None:
    ...
    if DATE_COLUMN in df.columns and df[DATE_COLUMN].isna().all():
        raise ValueError("snapshot_date exists but all values failed datetime parsing")
    if require_target and not set(df[TARGET_COLUMN].dropna().unique()).issubset({0, 1, "0", "1", False, True}):
        raise ValueError("stockout_event must be binary")
